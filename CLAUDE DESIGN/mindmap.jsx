...
const Mindmap = ({ data }) => {
  ...
  React.useEffect(() => {
    ...
    // sizes
    const rootW = Math.min(260, Math.max(180, (data.name.length * 10) + 40));
    const rootH = 54;
    const rootX = W / 2;
    const rootY = H / 2;
    ...
    const drawSide = (blocks, sideSign) => {
      blocks.forEach(b => {
        ...
        const bx = b.x - sideSign * (measureBranchWidth(b.node.name) / 2);
        linkG.append("path").attr("d", curve(rx, rootY, bx, b.yCenter));

        // branch → each leaf
        b.leafBlocks.forEach(lb => {
          const bx2 = b.x + sideSign * (measureBranchWidth(b.node.name) / 2);
    ...
    // measure branch pill width by text length (approximate)
    const measureBranchWidth = (text) => Math.min(240, Math.max(120, text.length * 8.2 + branchPillPadX * 2 + 26));
    ...
    const drawBranches = (blocks, sideSign) => {
      blocks.forEach(b => {
        const bw = measureBranchWidth(b.node.name);
